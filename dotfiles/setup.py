...
def _get_candidate_profile_files() -> List[Path]:
...
def _build_export_block(bin_dir: Path) -> str:
...
def install_path(bin_dir: Path) -> None:
    ...
    block = _build_export_block(bin_dir)
    updated_any = False
    created_in: List[Path] = []

    for profile in _get_candidate_profile_files():
        try:
            if profile.exists():
                original = profile.read_text(encoding="utf-8")
                without = _remove_marked_block(original)
                new_text = without
    ...
    # Update the current process PATH so immediate child processes can use it
...
def uninstall_path() -> None:
    ...
    for profile in _get_candidate_profile_files():
        ...
        try:
            original = profile.read_text(encoding="utf-8")
            new_text = _remove_marked_block(original)
            if new_text != original:
                profile.write_text(new_text, encoding="utf-8")
                removed_any = True
